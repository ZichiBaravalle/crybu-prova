...
export interface ClientOptions {
   ...
   *
   * Defaults to process.env['CRYBU_PROVA_BASE_URL'].
   */
   ...
   * Set the log level.
   *
   * Defaults to process.env['CRYBU_PROVA_LOG'] or 'warn' if it isn't set.
   */
...
/**
 * API Client for interfacing with the Crybu Prova API.
 */
export class CrybuProva {
  apiKey: string;
   ...
   * @param {string} [opts.baseURL=process.env['CRYBU_PROVA_BASE_URL'] ?? https://petstore3.swagger.io/api/v3] - Override the default base URL for the API.
   ...
   */
  constructor({
    baseURL = readEnv('CRYBU_PROVA_BASE_URL'),
    apiKey = readEnv('PETSTORE_API_KEY'),
    ...opts
  }: ClientOptions = {}) {
    if (apiKey === undefined) {
      throw new Errors.CrybuProvaError(
        "The PETSTORE_API_KEY environment variable is missing or empty; either provide it, or instantiate the CrybuProva client with an apiKey option, like new CrybuProva({ apiKey: 'My API Key' }).",
      );
    }
    ...
    this.timeout = options.timeout ?? CrybuProva.DEFAULT_TIMEOUT /* 1 minute */;
    ...
    this.logLevel =
      ...
      parseLogLevel(readEnv('CRYBU_PROVA_LOG'), "process.env['CRYBU_PROVA_LOG']", this) ??
      defaultLogLevel;
    ...
    const customHeadersEnv = readEnv('CRYBU_PROVA_CUSTOM_HEADERS');
    if (customHeadersEnv) {
  ...
  static CrybuProva = this;
  static DEFAULT_TIMEOUT = 60000; // 1 minute

  static CrybuProvaError = Errors.CrybuProvaError;
...
CrybuProva.Pets = Pets;
CrybuProva.Store = Store;
CrybuProva.Users = Users;

export declare namespace CrybuProva {
